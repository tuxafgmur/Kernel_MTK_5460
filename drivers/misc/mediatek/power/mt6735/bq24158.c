...
kal_uint32 bq24158_config_interface (kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    battery_xlog_printk(BAT_LOG_CRTI,"[bq24158_config_interface] write Reg[%x]=0x%x\n", RegNum, bq24158_reg);
...
{             
    ...
    //---------------------
  //  bq24158_hw_init();
  	bq24158_config_interface_liao(0x06,0x77); //yixuhong 20150508 add set ISAFE and HW CV point (4.36)
...
static ssize_t store_bq24158_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    if(buf != NULL && size != 0)
    {
        battery_xlog_printk(BAT_LOG_FULL,"[store_bq24158_access] buf is %s and size is  \n",buf/*,size*/);
