	...
	if (enable) {
		...
		if (!rmi4_data->irq_enabled) {
			...
			//mt_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, CUST_EINT_TOUCH_PANEL_DEBOUNCE_EN, CUST_EINT_POLARITY_LOW, , 1);
                        //@xuchunsheng changed start trigger mode from LOW_EDGE to LOW in 13/10/2015
			mt_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, EINTF_TRIGGER_LOW, tpd_eint_handler, 0);
                        //@xuchunsheng changed end
			}
		mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
...
*/
   ...
   // mt_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, CUST_EINT_TOUCH_PANEL_TYPE, tpd_eint_handler,0);
    mt_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, EINTF_TRIGGER_LOW, tpd_eint_handler,0);//@xuchunsheng changed the trigger condition from EINTF_TRIGGER_FALLING to EINTF_TRIGGER_LOW for solving touch invalid problem in 09/08/2015
    mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
}
...
#ifdef CONFIG_ARCH_MT6580
        CTP_DBG("synaptics_rmi4_probe zx\n");
		regulator_set_voltage(tpd->reg, 2800000, 2800000);  // set 1.8v
		regulator_enable(tpd->reg);  //enable regulator
#else
		hwPowerOn(TPD_POWER_SOURCE, VOL_2800, "TP");
...
//@xuchunsheng removed start for leakage current when tp enter in sleep in 08/29/2015 
//#ifdef CONFIG_ARCH_MT6580
//                regulator_disable(tpd->reg);
...
//@xuchunsheng removed end in 08/29/2015
	}
...
//@xuchunsheng added start for leakage current when tp enter in sleep in 08/29/2015 
#ifdef CONFIG_ARCH_MT6580
                regulator_disable(tpd->reg);
