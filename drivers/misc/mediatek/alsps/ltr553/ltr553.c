 ...
 * CUMULATIVE LIABILITY WITH RESPECT TO THE MEDIATEK SOFTWARE RELEASED HEREUNDER WILL BE,#define COMPAT_ALSPS_GET_PS_RAW_DATA			_IOR(ALSPS, 0x04, compat_int_t)
 ...
 * configuration
*******************************************************************************/
#define I2C_DRIVERID_LTR553 553
/*----------------------------------------------------------------------------*/
...
static int set_low_temperature_threshold_value(struct ltr553_priv *obj,int ps)
{
    ...
    APS_LOG(" set_low_temperature_threshold_value:read  obj->ps_thd_val_high=%x, obj->ps_thd_val_low=%x! \n",atomic_read(&obj->ps_thd_val_high), atomic_read(&obj->ps_thd_val_low));
...
static void low_temperature_value_all_reset(void)
{
    ...
    APS_LOG("low_temperature_value_all_reset: All values has been reset !");
...
static DRIVER_ATTR(enable_als,  S_IRUGO, ltr553_show_enable_als, ltr553_store_enable_als);
...
exit_handle:
    ...
    /*
    if((err = ap3220_enable_ps(ltr553_obj->client, 0)))
    {
...
#ifdef CONFIG_COMPAT
static long ltr553_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	long ret =0;
	void __user *arg32 = compat_ptr(arg);
	if (!file->f_op || !file->f_op->unlocked_ioctl)
		return -ENOTTY;
    printk("ltr553_compat_ioctl arg: 0x%lx, arg32: 0x%p\n",arg, arg32);
	switch (cmd) {			 
...
#ifdef CONFIG_COMPAT
	.compat_ioctl = ltr553_compat_ioctl,
#endif
};
/*----------------------------------------------------------------------------*/
